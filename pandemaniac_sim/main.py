...
def select_strategy(strategy_name, graph, cluster_flag, num_seeds, ratio, num_clusters, atmost_nodes_num, out_filename):
	...
	if strategy_name == "deg":
		final_nodes = degree_only(graph, num_seeds, ratio)
		write_out(final_nodes, out_filename)
	elif strategy_name == "clo":
		final_nodes = closeness_only(graph, num_seeds, ratio)
		write_out(final_nodes, out_filename)
	elif strategy_name == "bet":
		final_nodes = betweenness_only(graph, num_seeds, ratio)
		write_out(final_nodes, out_filename)
	elif strategy_name == "aggr":
		aggregate_centrality(graph, num_seeds, ratio, overlap_ratio, rounds, out_filename)


def degree_only(graph, num_seeds, ratio):
	'''
	...
	final_nodes = random_choice(num_seeds, ratio, selected_n_nodes)
	return final_nodes

def closeness_only(graph, num_seeds, ratio):
	'''
...
def betweenness_only(graph, num_seeds, ratio):
	'''
